...
"""
        ...
        for deplayerbranch in dependencies:
            ...
            logger.plain('  skip-recommends =  %s' % args.skip_recommends)
            ...
            for dep in layerDeps:
                if dep.required:
                    ...
                    if args.skip_recommends:
                        ...
                        ###logger.plain('  -------------->>>>>>>>  layerDeps %s' % layerDeps)
                        ###logger.plain('  -------------->>>>>>>>  args.layername %s' % args.layername)
                        ###logger.plain('  -------------->>>>>>>>  dependencies %s' % dependencies)
                        ###logger.plain('  -------------->>>>>>>>  dep %s' % dep)
                        ###logger.plain('  -------------->>>>>>>>  dep.layer %s' % dep.layer)
                        ###logger.plain('  -------------->>>>>>>>  dep.layer.name %s' % dep.layer.name)
                        ignore_layers.append(deplayerbranch)
                        ###ignore_layers.append(dep.layer.name)
                        ###ignore_layers.append("meta-dpdk")
                        ####ignore_layers.append(args.layername)
                    else:
